...
func (s *Spanner) init(ctx context.Context) error {
	...
	if s.edition != "ENTERPRISE" && s.edition != "ENTERPRISE_PLUS" {
		return fmt.Errorf("property graphs are not supported in %s edition", s.edition)
	}
	...
	if s.dbDialect != "GOOGLE_STANDARD_SQL" {
		return fmt.Errorf("property graphs are not supported with %s dialect", s.dbDialect)
	}
...
func generateSpannerPropertyGraphDDL(ctx context.Context, args DDLArgs) (string, error) {
	...
	if args.Drop {
		stmt.WriteString("DROP PROPERTY GRAPH ")
	...
	stmt.WriteString("CREATE OR REPLACE PROPERTY GRAPH ")
