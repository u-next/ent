...
func DDL(ctx context.Context, args DDLArgs) (string, error) {
	...
	// Add PropertyGraph DDL generation similar to views
	for _, pg := range args.PropertyGraphs {
		q, _ := CreatePropertyGraphFromSchema(pg, args.Dialect)
		p.Changes = append(p.Changes, &migrate.Change{
			Cmd:     q,
			Comment: fmt.Sprintf("Add %q property graph", pg.Name),
		})
	}
...
func PropertyGraphFromSchema(pg *PropertyGraph) *sqlpgq.PropertyGraphBuilder {
	...
	for _, nt := range pg.NodeTables {
		...
		if nt.Alias != "" {
			nodeBuilder.As(nt.Alias)
		}
		if nt.Key != nil && len(nt.Key.Columns) > 0 {
			nodeBuilder.Key(nt.Key.Columns...)
		}
		for _, label := range nt.Labels {
			labelBuilder := nodeBuilder.Label(label.Name)
			if label.Properties != nil && label.Properties.Type == PropertiesTypeDerived {
				props := make([]string, len(label.Properties.DerivedProperties))
				for i, prop := range label.Properties.DerivedProperties {
					props[i] = prop.Expression
				}
				labelBuilder.Properties(props...)
			}
		}
		builder.NodeTable(nodeBuilder)
	}

	for _, et := range pg.EdgeTables {
		edgeBuilder := sqlpgq.EdgeTable(et.TableName)
		if et.Alias != "" {
			edgeBuilder.As(et.Alias)
		}
		if et.Key != nil && len(et.Key.Columns) > 0 {
			edgeBuilder.Key(et.Key.Columns...)
		}
		if et.SourceKey != nil {
			edgeBuilder.SourceKey(et.SourceKey.Columns, et.SourceKey.ReferencedTable, et.SourceKey.ReferencedColumns...)
		}
		if et.DestinationKey != nil {
			edgeBuilder.DestinationKey(et.DestinationKey.Columns, et.DestinationKey.ReferencedTable, et.DestinationKey.ReferencedColumns...)
		}
		for _, label := range et.Labels {
			labelBuilder := edgeBuilder.Label(label.Name)
			if label.Properties != nil && label.Properties.Type == PropertiesTypeDerived {
				props := make([]string, len(label.Properties.DerivedProperties))
				for i, prop := range label.Properties.DerivedProperties {
					props[i] = prop.Expression
		...
		builder.EdgeTable(edgeBuilder)
	}

	return builder
}

// CreatePropertyGraphFromSchema creates a PropertyGraphBuilder from a schema.PropertyGraph with dialect support.
func CreatePropertyGraphFromSchema(pg *PropertyGraph, dialect string) (string, []any) {
	builder := PropertyGraphFromSchema(pg)
